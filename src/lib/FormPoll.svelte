...
<div
  ...
  class="absolute top-1/2 left-1/2 mt-2 max-h-[80vh] w-full max-w-md -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg border border-gray-200 bg-white p-6 shadow-lg transition-colors duration-300 dark:border-gray-700 dark:bg-gray-800"
  role="dialog"
  aria-labelledby="add-poll-title"
>
  <h2 id="add-poll-title" class="mb-4 text-xl font-bold text-gray-800 dark:text-gray-200">
    Add New Poll
  </h2>
  {#if errorMessage}
    <p
  ...
  <form onsubmit={handleCreating} class="flex flex-col gap-4">
    ...
    <div class="mt-4 flex gap-2">
      <button
        ...
        class="w-full rounded-lg bg-red-600 px-4 py-2 font-semibold text-white transition-colors duration-200 hover:cursor-pointer hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600"
